...
def pad_to_match_size(tensor_a, tensor_b):
    diff = tensor_b.size(-1) - tensor_a.size(-1)
    if diff > 0:
        tensor_a = F.pad(tensor_a, (0, diff))
    elif diff < 0:
        tensor_b = F.pad(tensor_b, (0, -diff))
...
def feature_loss(features_stft_disc_x, features_wave_disc_x, features_stft_disc_G_x, features_wave_disc_G_x, lengths_wave, lengths_stft):
    ...
    stft_loss = torch.stack([(lambda x, y: ((x - y).abs().sum(dim=-1) / lengths_stft[i].view(-1, 1, 1)).sum(dim=-1).sum(dim=-1))(*pad_to_match_size(feat_x, feat_G_x)) for i, (feat_x, feat_G_x) in enumerate(zip(features_stft_disc_x, features_stft_disc_G_x))], dim=1).mean(dim=1, keepdim=True)
    ...
    wave_loss = torch.stack([
        torch.stack([
            (lambda x, y: (x - y).abs().sum(dim=-1).sum(dim=-1) / lengths_wave[key][i])(*pad_to_match_size(feat_x, feat_G_x))
            for i, (feat_x, feat_G_x) in enumerate(zip(features_wave_disc_x[key], features_wave_disc_G_x[key]))
        ], dim=1)
        for key in wave_disc_names
    ], dim=2).mean(dim=1)
